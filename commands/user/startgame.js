                    ...
                    async (err, inventory) => {
                        ...
                        if(!character && !inventory){
                            ...
                            msg0.channel.awaitMessages({ max: 1, time: 10000, errors: ['time'] }).then(collected => {
                                ...
                                let name = collected.content();

                                const msg1 = await message.channel.send({ content: `${name}? Okay then.\nAnd how old would you like to be? (Full name)` });

                                msg1.channel.awaitMessages({ max: 1, time: 10000, errors: ['time'] }).then(collected => {

                                    if(isNaN(age)){
                                        m.reply({ content: 'no' })
                                    } else if(age > 15 || age < 35){
                                        m.reply({ content: 'Your age was either too young or too old. Start over and choose a valid age.' })
                                    } else {
